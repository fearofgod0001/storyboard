...
const SrchListItem = ({ item, onAddTestItem }) => {
  ...
  const _onAddTestItem = () => {
    onAddTestItem({ EX_IDX: item.EX_IDX, EX_DATA: itemData.EX_DATA, EX_TITLE: item.EX_TITLE, EX_TYPE: item.EX_TYPE });
  };
  ...
  return (
    <div className="list-item" ref={drag} style={{ opacity }}>
      <div className="item-title" onMouseDown={onHandleExData}>
        ...
        <div className="item-show">{isOpenData ? <UpOutlined /> : <DownOutlined />}</div>
      </div>
