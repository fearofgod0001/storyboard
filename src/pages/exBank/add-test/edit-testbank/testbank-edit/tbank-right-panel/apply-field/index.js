...
const ManagerInfoView = ({ fieldValue }) => {
  const { EX_CANDIDATE } = fieldValue || {};
  return (
    <StyledPanel>
      <div className="title">응시자</div>
      <div className="separator" />
      <div className="manager-panel">
        {EX_CANDIDATE &&
          EX_CANDIDATE.map((manager) => {
...
export const ApplyField = ({ value: fieldValue, onChange, pageMode }) => {
  ...
  const onSelectedUser = useCallback(
    (resultSet) => {
      ...
      if (isChange) {
        onChange({ EX_CANDIDATE: resultSet });
        setIsOpenCard(false);
      }
    },
    [onChange]
  ...
  return ['V'].includes(pageMode) ? (
    ...
    >
      <StyledApplyField>
        <div className="row">
          <div className="row-content">
            {EX_CANDIDATE && EX_CANDIDATE.length > 0 ? (
              EX_CANDIDATE?.map((user, i) => (
      ...
      <AntModal
        ...
        destroyOnClose
      >
        <UserSelector onSelectedUser={onSelectedUser} initUserList={EX_CANDIDATE} />
      </AntModal>
    </AntCard>
  );
};
